...
-- ==============================================
--use fpcapps
--drop proc bm.GenerateCrosses
CREATE OR ALTER PROC BM.[UploadBidPart_GenerateCrossesSegmented] (
	@BidId INT
	,@UserId VARCHAR(200)
	,@StartIndex INT
	,@EndIndex INT
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT DISTINCT 
			...
			,row_number() over(order by bp.BidPartID) as RowIndex
		--INTO #TempBidParts
		...
		SELECT 
			...
			,ManufacturerPartNumber
		INTO #TempBidParts
		FROM BidPartsCTE cte
		WHERE cte.RowIndex >= @StartIndex AND cte.RowIndex <= @EndIndex
		...
		BEGIN
			...
			AND cpOld.Definition <> 'DoNotOffer'
	   ...
	   END
		...
		------------------------------------------------------------------------------------------------------------------------------------------------
		-----------------if the part numbers do not match exactly then match any 5 consecutive characters of the part numbers---------------------------
		------------------------------------------------------------------------------------------------------------------------------------------------

		 IF OBJECT_ID('tempdb..#TempCrossPart') IS NOT NULL
         DROP TABLE #TempCrossPart;
		...
		IF EXISTS (
			...
			BEGIN
				...
				;with cteECatalogCrosses(
					...
					,'A'
					,'system'
					,ROW_NUMBER() OVER (PARTITION BY prod.PartNumber)
				FROM #TempBidParts bp
				INNER JOIN [FPCAPPS].[DBO].[ECatalogCrosses] ECat WITH (NOLOCK) ON 
					BM.SmartMatch(ECat.FromPartNo, BP.CleanManufacturerPartNumber) = 1
						OR BM.SmartMatch(ECat.FromPartNo, BP.CleanCustomerPartNumber) = 1
				INNER JOIN [FPBIDW].[EDW].[DimProduct] prod WITH (NOLOCK) ON  bp.Company = prod.Company and prod.partnumber = Ecat.ToPartNo  AND prod.pool = ecat.toPoolNo and prod.pool not in ('555','556','0') 
				LEFT JOIN [FPBIDW].[EDW].[DimVendor] dv WITH (NOLOCK) ON dv.vendorkey = prod.vendorkey
				LEFT OUTER JOIN FPCAPPS.BM.VendorGrade vg WITH (NOLOCK) ON dv.VendorNumber = cast(vg.VendorNumber AS DECIMAL(9, 0))
					AND prod.Category = vg.Category 
